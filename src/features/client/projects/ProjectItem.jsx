...
function ProjectItem({ project }) {
  ...
  return (
    <Card className="group/project-card w-full p-0 flex flex-col justify-between border border-zinc-200/20 dark:border-zinc-800/20 bg-card/60 backdrop-blur-md hover:border-primary/20 dark:hover:border-primary/20 hover:shadow-[0_0_20px_rgba(16,185,129,0.02)] transition-all duration-300 h-full min-h-[250px] relative overflow-hidden">
      <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-transparent via-primary/30 to-transparent opacity-0 group-hover/project-card:opacity-100 transition-opacity duration-300" />
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center text-xs">
            <span className="font-medium text-foreground/80 flex items-center gap-1">
              <FaChartLine className="size-3 text-primary shrink-0" />
              <span>Progress</span>
            </span>
          ...
          <Progress 
            ...
            indicatorClassName={
              ...
              project.progress > 50 ? "bg-primary shadow-[0_0_8px_rgba(16,185,129,0.3)]" :
              "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.3)]"
            }
          />
        </div>

        <div className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded-lg border border-zinc-200/20 dark:border-zinc-800/20 bg-muted/10">
          <span className="text-muted-foreground flex items-center gap-1 font-semibold uppercase tracking-wider text-[10px]">
            <FaWallet className="size-3 text-primary/70 shrink-0" />
      ...
      <div className="p-5 pt-0 mt-auto border-t border-zinc-200/20 dark:border-zinc-800/20 bg-muted/5">
        <Button variant="outline" className="w-full h-9 group-hover/project-card:bg-primary group-hover/project-card:text-primary-foreground group-hover/project-card:border-primary transition-all duration-300 gap-1.5 cursor-pointer mt-4" asChild>
          <Link href={`/projects/${project.id}`}>
            ...
            <FaArrowRight className="size-3 transition-transform duration-300 group-hover/project-card:translate-x-1" />
          </Link>
        </Button>
      </div>
    </Card>
